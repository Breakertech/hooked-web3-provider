...
export default class HookedWeb3Provider extends Web3.providers.HttpProvider {
  ...
  send(payload, callback) {
    ...
    for (const request of requests) {
      if (request.method === 'eth_sendTransaction') {
        throw new Error(
          'HookedWeb3Provider does not support synchronous transactions. Please provide a callback.'
        );
      }
    }

    const finishedWithRewrite = () => super.send(payload, callback);
  ...
  rewritePayloads(index, requests, session_nonces, finished) {
    ...
    this.transaction_signer.hasAddress(sender, (err, has_address) => {
      ...
      const getNonce = done => {
        ...
        const nonce = session_nonces[sender];
        if (nonce != null) {
          done(null, nonce);
        } else {
      ...
      getNonce((err, nonce) => {
        ...
        const final_nonce = Math.max(nonce, this.global_nonces[sender] || 0);
